...
describe("levelToMetersForRoom", () => {
  ...
  // partition into a completely different room — a wall bordering the
  // room only across part of its own run clips down to that sub-span, so
  // an isolated single-room 3D view never renders (or reports área/volume
  // for) the OTHER room's unrelated portion of a wall it merely shares.
  const level = {
    sketchScale: "0.5",
    elevation: 0,
    sketchElements: [
