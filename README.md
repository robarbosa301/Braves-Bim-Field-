...
7. **Vista isolada / explodida** (botão no topo, aparece com um elemento selecionado): mostra só aquele elemento numa explosão vertical, e cada **sub-componente real** fica no seu próprio nível — não é só "fôrma/concreto/armadura" em bloco:
   - Sapata: fôrma do bloco da base, fôrma do tronco de pirâmide, cada grupo de armadura (ex. malha inferior, malha superior — quando o projeto tem os dois), concreto do bloco da base, concreto do tronco — cada um com seu próprio volume/área/sacos de cimento.
   ...
   Cada nível tem um rótulo flutuante só com os números daquele pedaço (dimensões, volume, sacos de cimento, m³ de areia/brita, quantidade/diâmetro/comprimento/peso das barras). A lista de grupos de armadura vem de `q.armadura.grupos` (o mesmo dado usado na aba Quantitativos), então funciona automaticamente pra qualquer quantidade de grupos, tanto em elementos importados do IFC quanto criados manualmente. A câmera se ajusta sozinha. Bom pra conferir de perto qualquer peça antes/depois de executar.
...
- **Pilar de arranque × viga baldrame — indicador de continuação**: no IFC, o pilar de arranque termina no nível de referência do pavimento, mas a viga baldrame nasce mais acima (confirmado cruzando com os desenhos de fôrma do projeto: a "Elevação" da viga bate exatamente com essa diferença). Ou seja, na obra real existe um lance de pilar entre o topo do arranque e a viga que não é modelado como parte deste elemento no IFC (pertence a outro trecho do pilar, fora do escopo atual). Para não inflar o volume de concreto/fôrma do arranque com um trecho que não é dele, o app **não altera a geometria/quantitativos do arranque** — em vez disso desenha esse trecho como um volume sólido cinza translúcido entre o topo do arranque e a viga, dando a continuidade visual real do pilar (e mostra a distância na aba Editar). Os quantitativos do arranque continuam batendo só com o que ele realmente é; o volume de continuidade é só visual, não entra em nenhum quantitativo.
